...
| 9   | Vitrina: cada recuadro aparece levemente uno a uno; el número crece y vuelve                    | `Stagger as="ul"` con `StaggerItem as="li"` y `fadeInSlow` (140 ms); `CifraQueLlama` sobre el chip entero de la cuenta (1 → 1,08 → 1, ease-out-cubic + ease-in-out-cubic, sin rebote; la primera versión escalaba solo el texto con ease-out-back y el dueño la rechazó: «torpe y de mal gusto»). La caja de frente ya no trae su `<li>`: lo pone quien la lista |
