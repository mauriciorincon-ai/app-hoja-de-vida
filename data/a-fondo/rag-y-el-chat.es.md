...
Los chips que acompañan una respuesta llevan las cuatro fuentes que la recuperación entregó al modelo —dos fragmentos del mismo documento comparten un chip, con sus dos números—, y cada uno navega a su sección del sitio. No existe hoy un validador de salida que compruebe que el modelo usó cada fuente citada en cada frase; existe la prueba de que ninguna cita puede apuntar a un destino inexistente, y existe el prompt que obliga al modelo a numerar las fuentes que utiliza. La relación entre la afirmación y la evidencia la verifica hoy quien lee, con el destino a un clic.
