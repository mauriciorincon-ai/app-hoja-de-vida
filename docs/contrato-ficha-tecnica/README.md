...
| `ficha-tecnica.schema.json`          | El contrato **v1.3.1** como JSON Schema (v1.3.0: `conclusiones` y `galeria`, opcionales; v1.3.1: la ruta de una captura no puede escaparse con `..` ni empezar en `/`). **Generado** del Zod de la app (`pnpm contrato:ficha`); el test `ficha-tecnica-contrato` exige que coincidan.                     |
