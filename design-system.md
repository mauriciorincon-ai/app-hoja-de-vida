...
| `CifraQueLlama` | post-S8 — el CHIP entero (fondo, borde, texto) crece a 1.08× y vuelve en una curva limpia: ease-out-cubic al subir (0.28s) + ease-in-out-cubic al asentarse (0.42s), 0.55s de retraso, al asomar (`amount: all`). Sin overshoot. Solo transform (cuenta de productos) |
...
- **Vitrina asomada**: las mismas cajas de frente del portal, sin variante, con un botón sage
  ...
  «piezas», en todo el copy visible— crece y vuelve al asomar como chip entero, sin rebote (`CifraQueLlama`), y el cierre de
