...
| `CifraQueLlama` | post-S8 — el CHIP entero (fondo, borde, texto) crece a 1.3× y vuelve **al mismo tiempo que aparece su tarjeta**: misma duración que `fadeInSlow` (1.2s), mismo escalón (viaja como prop `retraso`, porque el `staggerChildren` del `Stagger` no llega a los nietos — medido), ease-in-out-cubic simétrico, origen en su borde derecho; se repite en cada pasada como toda entrada. Sin disparador propio, sin rebote. Solo transform (cuenta de productos) |
...
- **Vitrina asomada**: las mismas cajas de frente del portal, sin variante, con un botón sage
  ...
  «piezas», en todo el copy visible— crece a 1.3× al mismo tiempo que aparece su caja —misma duración, mismo escalón— en cada pasada, como chip entero y sin rebote (`CifraQueLlama`), y el cierre de
