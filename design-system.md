...
| `CifraQueLlama` | post-S8 — el CHIP entero (fondo, borde, texto) crece a 1.3× y vuelve, **una sola vez, cuando su tarjeta terminó de aparecer**: hereda la variante `visible` del `StaggerItem` con 1.15s de retraso (la entrada `fadeInSlow` dura 1.2s), 0.7s ease-in-out-cubic simétrico, origen en su borde derecho. Sin disparador propio, sin rebote. Solo transform (cuenta de productos) |
...
- **Vitrina asomada**: las mismas cajas de frente del portal, sin variante, con un botón sage
  ...
  «piezas», en todo el copy visible— crece a 1.3× una sola vez cuando su tarjeta terminó de aparecer, como chip entero y sin rebote (`CifraQueLlama`), y el cierre de
